...
namespace AutoLevelup
{
    class Program
    {
        ...
        {
            AMenu = MainMenu.AddMenu("AutoLevelUp", "ALU");
            AMenu.AddGroupLabel("Welcome to my AutoLevelUp Addon!");
            AMenu.Add("delay", new Slider("Randomize max delay value", 1000, 0, 5000));
        }
        ...
        {
            Smite = ObjectManager.Player.GetSpellSlotFromName("SummonerSmite");
...
tipo = " Lane";
            ...
            Chat.Print("Auto LevelUp by smjeon314");
            Chat.Print(Player.BaseSkinName + tipo + " Sequence Loaded");
            Load();
        }
        ...
        {//AutoLevelup
            ...
            if (qL + wL + eL + rL < ObjectManager.Player.Level)
            {
                ...
                if (qL < level[0]) up(SpellSlot.Q);
                if (wL < level[1]) up(SpellSlot.W);
                if (eL < level[2]) up(SpellSlot.E);
                if (rL < level[3]) up(SpellSlot.R);
            }


}
        static void up (SpellSlot s)
        {
            EloBuddy.SDK.Core.DelayAction(() =>
            {
                Player.Spellbook.LevelSpell(s);
            }, new Random().Next(0, AMenu["delay"].Cast<Slider>().CurrentValue));
        }
    }
}